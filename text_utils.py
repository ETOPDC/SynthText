...
class RenderFont(object):
    ...
    def render_multiline(self, font, text):
        ...
        fsize = (round(1.5 * line_bounds.width), round(3.0 * line_spacing * len(lines)))
    ...
    def render_sample(self, font, mask):
        ...
        while i < self.max_shrink_trials and max_font_h > self.min_font_h:
            ...
            # render the text:
            # copy from https://github.com/ankush-me/SynthText/issues/48
...
class FontState(object):
    ...
    def __init__(self, data_dir='data'):
        ...
        with open(char_freq_path, 'rb') as f:
            "这一块出现问题的原因是model下的文件，有的是用cPickle.dump()的str文件，有的是用pickle打包的是二进制文件"
            # pickle.dump()  --->  xxx.cp
        ...
        "这一块出现问题是因为 Python 的 open 方法默认编码取决于平台，如果是 Windows 平台，默认编码是 gbk，如果文件是 utf-8 编码，就会报这个错误。 "
        # self.fonts = [os.path.join(data_dir,'fonts',f.strip()) for f in open(self.FONT_LIST)]  # linux
        self.fonts = [os.path.join(data_dir, 'fonts', f.strip()) for f in open(self.FONT_LIST, encoding="UTF-8")]  # windows
